...
<form class="tasks-search2" action="{{ action('JobsListSearchController@show')}}">
...
<div class="jobs-short-profile">
  ...
  <div class="jobs-short-info">
    <h5><a href="/Bitsource/public/job/{{$task_table->id}}">{{$task_table->adTitle}}</a></h5>
    ...
    <h6 style="text-align:justify;">{{str_limit($task_table->adDesc, 200)}}</h6>
    <div class="jobs-short-glyphicons">
      <span class="glyphicon glyphicon-map-marker"></span><span class="space">{{$task_table->adCity}}</span>
      <span class="glyphicon glyphicon-briefcase"></span><span class="space">

      @if($task_table->adType == 'pliris')
      Πλήρης Απασχόληση</span>
      @else
      Μερική Απασχόληση</span>
      @endif
